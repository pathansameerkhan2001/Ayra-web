...
export function ExploreByOccasion({
  ...
  const [items, setItems] = useState<OccasionItem[]>(
    initialOccasions || DEFAULT_OCCASIONS
  );
    ...
    async function load() {
      ...
      try {
        ...
        if (data && data.length >= 6) {
          const mapped: OccasionItem[] = data.map((c) => ({
            id: c.id,
            name: c.name,
            slug: c.slug,
            image: c.image_url || `/images/occasions/${c.slug}.jpg`,
            alt: `Luxury ${c.name} gift hamper`,
          }));
          setItems(mapped);
        }
      } catch (err) {
  ...
  return (
    ...
    >
      {/* Decorative Botanical Left Branch */}
      <div
        aria-hidden="true"
      ...
      {/* Decorative Botanical Right Branch */}
      <div
        aria-hidden="true"
      ...
      <div className="max-w-[1380px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="w-full max-w-6xl mx-auto">
          ...
          >
            {items.map((occasion) => {
              const imageUrl = getStoragePublicUrl(occasion.image);
              return (
                <div
                  key={occasion.id}
                  role="listitem"
                  ...
                  >
                    ...
                    <div className="relative w-[112px] h-[112px] sm:w-[120px] sm:h-[120px] md:w-[134px] md:h-[134px] lg:w-[148px] lg:h-[148px] xl:w-[152px] xl:h-[152px] rounded-full overflow-hidden aspect-square bg-[#FAF7F2] shadow-[0_5px_18px_rgba(217,136,109,0.14)] ring-1 ring-ayra-blush-200/80 transition-all duration-500 ease-out group-hover:shadow-[0_10px_28px_rgba(217,136,109,0.25)] group-hover:ring-ayra-rose-light">
                      <Image
                        ...
                        alt={occasion.alt}
                        fill
                        sizes="(max-width: 640px) 112px, (max-width: 768px) 120px, (max-width: 1024px) 134px, 150px"
                    ...
                    {/* Centered Decorative Horizontal Underline */}
                    <span
                      aria-hidden="true"
