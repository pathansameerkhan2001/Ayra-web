...
import { getActiveReels } from "@/services/instagram";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-8 sm:mb-12">
          <div className="flex items-center justify-center gap-2 mb-2">
            <Instagram className="w-4 h-4 text-ayra-rose" />
          ...
          <h2 className="font-serif text-2xl sm:text-3xl md:text-4xl text-ayra-charcoal font-normal">
            Follow the Ayra Moments
          </h2>
          <p className="font-sans text-xs sm:text-sm text-ayra-charcoal/70 mt-1.5 font-light">
            Watch our handcrafted hampers come to life. Tag{" "}
            <span className="font-medium text-ayra-rose-medium">@ayrahampers</span> to be featured.
          </p>
        </div>

        {/* Reels Grid on Desktop (4-6 columns) / Swipe Carousel on Mobile */}
        <div className="flex sm:grid sm:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-4 lg:gap-5 overflow-x-auto sm:overflow-x-visible no-scrollbar snap-x snap-mandatory px-2 sm:px-0 py-2">
          {reels.map((reel) => (
            ...
            >
              ...
              {/* Gradient Overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-ayra-charcoal/80 via-transparent to-black/20 opacity-80 group-hover:opacity-90 transition-opacity" />

              {/* Top Instagram & Link Badges */}
              <div className="absolute top-2.5 inset-x-2.5 flex items-center justify-between text-white/90">
                <span className="p-1 rounded-full bg-black/30 backdrop-blur-md">
                  <Instagram className="w-3.5 h-3.5" />
                </span>
                <span className="p-1 rounded-full bg-black/30 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div className="text-center mt-8 sm:mt-10">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-white border border-ayra-blush-200 text-ayra-charcoal text-xs uppercase tracking-widest font-semibold hover:bg-ayra-peach-50 hover:border-ayra-rose transition-all shadow-subtle hover:shadow-luxury group"
          >
            <Instagram className="w-4 h-4 text-ayra-rose" />
            <span>Follow @ayrahampers</span>
          </a>
        </div>
      </div>
    </section>
